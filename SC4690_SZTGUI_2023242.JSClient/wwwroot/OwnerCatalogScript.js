...
function display() {
    ...
    owners.forEach(t => { document.getElementById('resultarea').innerHTML += "<tr><td> " + t.age + "</td><td> " + t.name + "</td><td> " + t.phoneNumber + "</td><td> " + t.ownerID + "</td><td> " + t.salary + "</td><td>" + `<button type="button" onclick="remove(${t.ownerID})">Delete</button> ` + ` <button type="button" onclick="showupdate(${t.ownerID})">Update</button>` + "</td></tr>" });
}
...
function showupdate(id) {
    document.getElementById('owneragetoupdate').value = owners.find(t => t['ownerID'] == id)['age'];
    document.getElementById('ownernametoupdate').value = owners.find(t => t['ownerID'] == id)['name'];
    document.getElementById('ownerphonenumbertoupdate').value = owners.find(t => t['ownerID'] == id)['phoneNumber'];
    document.getElementById('owneridtoupdate').value = owners.find(t => t['ownerID'] == id)['ownerID'];
    document.getElementById('ownersalarytoupdate').value = owners.find(t => t['ownerID'] == id)['salary'];
...
function update() {
    ...
    fetch("http://localhost:25418/owner", {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ age: age, name: name, phoneNumber: phoneNumber, ownerID: ownerID, salary: salary, ownerID: ownerIDtoUpdate }),
    })
        .then(response => {
            if (!response.ok) {
